...
const GOLES_DATA = [
  { r:1, n:"L. Messi",     t:"Argentina",    f:"ar", v:6 },
  { r:2, n:"K. Mbappé",    t:"Francia",      f:"fr", v:4 },
  { r:2, n:"Vinícius Jr.", t:"Brasil",       f:"br", v:4 },
  { r:2, n:"O. Dembélé",   t:"Francia",      f:"fr", v:4 },
  { r:2, n:"E. Haaland",   t:"Noruega",      f:"no", v:4 },
  { r:6, n:"Y. Wissa",     t:"Congo DR",     f:"cd", v:3 },
  { r:6, n:"H. Kane",      t:"Inglaterra",   f:"gb-eng", v:3 },
  { r:6, n:"I. Sarr",      t:"Senegal",      f:"sn", v:3 },
  { r:6, n:"J. David",     t:"Canadá",       f:"ca", v:3 },
  { r:6, n:"D. Undav",     t:"Alemania",     f:"de", v:3 },
];

const ASIST_DATA = [
  { r:1, n:"A. Isak",      t:"Suecia",       f:"se", v:3 },
  { r:1, n:"B. Guimarães", t:"Brasil",       f:"br", v:3 },
  { r:1, n:"M. Olise",     t:"Francia",      f:"fr", v:3 },
  { r:4, n:"F. Wirtz",     t:"Alemania",     f:"de", v:2 },
  { r:4, n:"J. Kimmich",   t:"Alemania",     f:"de", v:2 },
  { r:4, n:"M. Salah",     t:"Egipto",       f:"eg", v:2 },
  { r:4, n:"B. Saka",      t:"Inglaterra",   f:"gb-eng", v:2 },
  { r:4, n:"M. Ødegaard",  t:"Noruega",      f:"no", v:2 },
  { r:4, n:"K. Mbappé",    t:"Francia",      f:"fr", v:2 },
  { r:4, n:"H. Aouar",     t:"Argelia",      f:"dz", v:2 },
];

const EQUIPOS_DATA = [
  { n:"Francia",      f:"fr",     g:12, a:8 },
  { n:"Brasil",       f:"br",     g:11, a:7 },
  { n:"Alemania",     f:"de",     g:10, a:7 },
  { n:"Países Bajos", f:"nl",     g:9,  a:6 },
  { n:"Argentina",    f:"ar",     g:9,  a:4 },
  { n:"Suiza",        f:"ch",     g:9,  a:5 },
  { n:"Inglaterra",   f:"gb-eng", g:8,  a:5 },
  { n:"Suecia",       f:"se",     g:7,  a:6 },
  { n:"Portugal",     f:"pt",     g:7,  a:3 },
  { n:"Colombia",     f:"co",     g:5,  a:3 },
];

function GolazoStats() {
  const [statsTab, setStatsTab] = useState<'goles'|'asist'|'equipos'>('goles');
  ...
  const renderPlayerRow = (d: typeof GOLES_DATA[0], max: number, color: string, i: number) => (
    <div key={i} style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 0', borderBottom:'0.5px solid #f3f4f6' }}>
      <span style={{ fontSize:13, fontWeight:700, minWidth:20, textAlign:'center', color: d.r===1?'#b8860b':d.r<=3?'#888':'#ccc' }}>{d.r}</span>
      <img src={`https://flagcdn.com/20x15/${d.f}.png`} alt={d.t} width={20} height={15} className="rounded-sm" />
      <div style={{ flex:1 }}>
        <div style={{ fontSize:12, fontWeight:600 }}>{d.n}</div>
        <div style={{ fontSize:10, color:'#9ca3af' }}>{d.t}</div>
        {renderBar(d.v, max, color)}
      </div>
      <div style={{ fontSize:18, fontWeight:700, minWidth:24, textAlign:'right' }}>{d.v}</div>
    </div>
  );
    ...
    <div className="mb-6">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-400 font-normal">Fase de grupos</span>
        </div>
      ...
      {open && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="grid grid-cols-4 gap-0 border-b border-gray-100">
            {[['96','Partidos'],['249','Goles'],['2.6','Prom.'],['48','Equipos']].map(([v,l],i) => (
          ...
          <div className="flex border-b border-gray-100">
            {(['goles','asist','equipos'] as const).map(t => (
              <button key={t} onClick={() => setStatsTab(t)}
                ...
                {t === 'goles' ? 'Goleadores' : t === 'asist' ? 'Asistencias' : 'Equipos'}
              </button>
            ))}
          </div>
          ...
          <div className="px-4 py-2">
            {statsTab === 'goles' && GOLES_DATA.map((d,i) => renderPlayerRow(d, 6, '#c8102e', i))}
            {statsTab === 'asist' && ASIST_DATA.map((d,i) => renderPlayerRow(d, 3, '#1a6fb5', i))}
            {statsTab === 'equipos' && EQUIPOS_DATA.map((e,i) => (
              <div key={i} style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 0', borderBottom:'0.5px solid #f3f4f6' }}>
                <span style={{ fontSize:13, color:'#ccc', minWidth:20, textAlign:'center' }}>{i+1}</span>
                <img src={`https://flagcdn.com/20x15/${e.f}.png`} alt={e.n} width={20} height={15} className="rounded-sm" />
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:12, fontWeight:600 }}>{e.n}</div>
                  <div style={{ display:'flex', gap:4, marginTop:2 }}>
                    <span style={{ fontSize:10, padding:'1px 6px', borderRadius:3, background:'#dcfce7', color:'#15803d', fontWeight:700 }}>⚽ {e.g}</span>
                    <span style={{ fontSize:10, padding:'1px 6px', borderRadius:3, background:'#dbeafe', color:'#1d4ed8', fontWeight:700 }}>🎯 {e.a}</span>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="px-4 py-2 border-t border-gray-100">
            <p className="text-xs text-gray-400">Fuente: ESPN CO · Copa Mundial FIFA 2026</p>
          </div>
        </div>
      )}
    </div>
